...
import { AIUnlockAnimation } from "@/components/uitripled/animation";
...
type Phase = "loading" | "classifying" | "finalizing" | "done";
...
function ProcessingInner() {
	...
	useEffect(() => {
		...
		intervalRef.current = setInterval(() => {
			...
			setLiveSeries((prev) => [...prev.slice(-60), { time: Date.now() / 1000, value: next }]);
		}, 200);

		(async () => {
			try {
				const created: Complaint = await api.createComplaint({
				...
				const finalResult: Result = {
					...
					departmentName: dept?.name ?? "Public Services",
				...
				<div>
					...
					<h1 className="font-heading text-3xl tracking-tight">
						{phase === "done"
							...
							: "AI is analysing your submission"}
					</h1>
					{result && (
						<p className="mt-1 text-sm text-muted-foreground">
							Reference: <span className="font-mono">{result.referenceNumber}</span>
						</p>
					)}
				</div>
