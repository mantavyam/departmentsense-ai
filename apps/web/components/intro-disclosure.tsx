...
function StepContent({
  ...
  const renderActionButton = (action: Step["action"]) => {
    ...
    if (action.href) {
      return (
        <Button className="w-full " size="sm" variant="link" render={<a href={action.href} target="_blank" rel="noopener noreferrer" />} nativeButton={false}><span className="flex items-center gap-2">
                        {action.label}
