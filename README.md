...
AI-powered citizen grievance classification system. Citizens submit complaints, an NLP backend classifies them across **92 government departments** seeded from `docs/92-departments.json`, assigns a priority, and routes to the correct department head in real time.
...
| `ADMIN_EMAIL` | no | `admin@gov.in` | Email seeded as the administrator account on first DB init. |
| `ADMIN_PASSWORD` | **yes (prod)** | `ChangeMe-2026!` | Password for the seeded admin. **Override before first boot in production.** Bcrypt-hashed at seed time. |
| `ADMIN_NAME` | no | `System Administrator` | Display name for the admin account. |
...
Only **one** account is seeded — the administrator. Everything else is real:

| Role | How they get in |
|------|-----------------|
| **Administrator** | Seeded from `ADMIN_EMAIL` + `ADMIN_PASSWORD` env vars on the very first DB init. Defaults: `admin@gov.in` / `ChangeMe-2026!`. **Override these in `backend/.env` before first boot in production.** |
| **Citizen** | Self-signs up at `/auth` (citizen → Create one). Email + password (min 8 chars, letter + digit). |
| **Department Head** | No signup. Created automatically when admin adds a department with an officer email. Admin then generates a 6-digit numeric verification code on the Departments page and shares it out-of-band. Sign-in is **email + code only** — no password. |

The 92 departments are seeded from `docs/92-departments.json` and have **no verification codes** until the admin generates them per-department from the Departments page.

> ⚠️ The seed runs once. If you change `ADMIN_PASSWORD` after first boot, delete `backend/departmentsense.db` and restart so the seed re-runs.
...
**Admin password changes have no effect**
The admin is seeded once. Delete `backend/departmentsense.db` after editing `ADMIN_PASSWORD` in `backend/.env` to re-seed.

**Department head can't sign in**
A 6-digit verification code must first be generated by the admin from `/dashboard/departments` (click the department card → Generate code). The code is shared with the head out-of-band; they sign in at `/auth` with their email + that code (no password).
